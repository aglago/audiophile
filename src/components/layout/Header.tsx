...
export const Header = () => {
  ...
  return (
    <header className={`text-white w-full ${pathname === "/" ? "absolute top-0" : "bg-black"}`}>
      <div className="max-w-7xl mx-auto border-b border-white" style={{ borderColor: "rgba(255, 255, 255, 0.2)" }}>
        <div className="flex items-center justify-between py-8 px-6">
          ...
          <div className="flex items-center space-x-4">
            <Button variant="ghost" size="sm" className="hidden sm:flex text-white hover:bg-gray-800">
            ...
            <nav className="py-8 px-6 space-y-2">
              {navigation.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="block px-4 py-2 text-sm font-medium tracking-wide hover:text-primary hover:bg-gray-800 transition-colors duration-200"
